...
- [ ] **More crops / flavor variants** — the multi-crop system (crop-aware `TileGrid`, per-crop `crops.json` entries) currently only has apple/hops/coffee/timber. `docs/GameDesign.md` §3.4 names unused botanical zones (Peach Orchard, Cherry Grove, Blueberry Bend, etc.) and §8.1 names cider flavor variants (Autumn Hug, Creekside Watermelon, Sticky Paws Apple Caramel, ...) that were planned but never wired to a recipe.
